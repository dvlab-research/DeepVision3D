...
from mmdet3d.models.model_utils import AuxModule
...
class GroupFree3DHead(BaseModule):
    ...
    def __init__(self,
        ...
        # Build auxiliary supervision module (optional).
        if aux_module_cfg is not None:
            # Define auxiliary module.
            self.aux_module = AuxModule(num_classes=self.num_classes,
                                        **aux_module_cfg)
        else:
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        # compute auxiliary loss for seed points. (optional)
        if self.training and self.aux_module is not None:
            aux_vote_points, aux_class_points = self.aux_module(
                seed_xyz, feat_dict['fp_interval_features'][-1])
            results.update({
                'aux_vote_points': aux_vote_points,
                'aux_class_points': aux_class_points,
            })
    ...
    def loss(self,
        ...
        # calculate auxiliary loss (optional)
        if self.aux_module is not None:
            aux_targets, aux_target_masks = self.aux_module.get_targets(
                points, self.bbox_coder.with_rot, gt_bboxes_3d, gt_labels_3d, pts_semantic_mask, pts_instance_mask)
            aux_vote_loss, aux_class_loss = self.aux_module.get_loss(
                bbox_preds['seed_points'],
                bbox_preds['aux_vote_points'],
                bbox_preds['aux_class_points'],
                bbox_preds['seed_indices'],
                aux_target_masks, aux_targets)
            losses['aux_vote_loss'] = aux_vote_loss
            losses['aux_class_loss'] = aux_class_loss
