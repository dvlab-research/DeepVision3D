...
__global__ void knn_kernel(
    ...
    const float *__restrict__ new_xyz, const int *__restrict__ new_xyz_cnt,
    ...
    // determine bs_idx.
    int new_xyz_cumsum = 0;
    int bs_idx;
    for (bs_idx = 0; bs_idx < b; bs_idx++){
        new_xyz_cumsum += new_xyz_cnt[bs_idx];
        if (pt_idx < new_xyz_cumsum){
            break;
        }
        xyz += xyz_cnt[bs_idx] * 3;
    }
    ...
    for(int i = 0; i < xyz_cnt[bs_idx]; i++){
        float x = xyz[i * 3 + 0];
        float y = xyz[i * 3 + 1];
        float z = xyz[i * 3 + 2];
...
void knn_kernel_launcher(
    int b, int n, int m, int nsample,
    const float *xyz, const int *xyz_cnt,
    const float *new_xyz, const int *new_xyz_cnt,
    int *idx, float *dist2, cudaStream_t stream) {
    // param new_xyz: (m1 + m2 + ..., 3)
    // param xyz: (n1 + n2 + ..., 3)
    // param idx: (m1 + m2 + ..., nsample)
    ...
    knn_kernel<<<blocks, threads, 0, stream>>>(
        b, n, m, nsample, xyz, xyz_cnt, new_xyz, new_xyz_cnt, idx, dist2);
