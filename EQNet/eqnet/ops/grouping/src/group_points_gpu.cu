...
__global__ void group_points_grad_kernel_stack(int B, int M, int C, int N, int nsample,
    ...
    int in_feature_batch_cnt = features_batch_cnt[bs_idx];
    int in_idx = (idx[0] % in_feature_batch_cnt + in_feature_batch_cnt) % in_feature_batch_cnt;
    grad_features += (features_batch_start_idx + in_idx) * C + C_idx;
...
__global__ void group_points_kernel_stack(int B, int M, int C, int nsample,
    ...
    idx += pt_idx * nsample + sample_idx;
    // If sample_idx is -1, return the features of last point in this batch.
    int in_feature_batch_cnt = features_batch_cnt[bs_idx];
    int in_idx = (idx[0] % in_feature_batch_cnt + in_feature_batch_cnt) % in_feature_batch_cnt;
    in_idx = in_idx * C + C_idx;
