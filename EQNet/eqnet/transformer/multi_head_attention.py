...
class MultiheadAttention(Module):
    ...
    def forward(
        ...
        v = v.contiguous().view(-1, self.num_heads, self.head_dim)

        # compute attention weight.
        attn_output_weights = self.attention_utils.attention_weight_computation(
        ...
        if ctx_rpe_value is not None:
            ...
            attn_output = self.attention_utils.attention_value_computation(
                query_batch_cnt, key_batch_cnt, index_pair_batch, index_pair,
                attn_output_weights, v)
        assert list(attn_output.size()) == [total_query_len, self.num_heads, self.head_dim]

        attn_output = attn_output.view(total_query_len, embed_dim)
